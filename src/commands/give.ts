            ...
            .then(u => {
                ...
                if (args.length > 2 || swap) {
                    ...
                    if (!force) {
                        ...
                        if (!item) {
                            if (swap) repl(mes.channel, mes.author, `${args[1]} is an invalid amount of cookies.`, 'bad');
                            else repl(mes.channel, mes.author, `Item ${args[1]} not found!`, 'bad');
                            resolve(false);
                            return;
                        }
                        if (!item.tradeable) {
                        ...
                        if (!iitem || !iitem.amount) {
                            if (item.expanded) repl(mes.channel, mes.author, `You have to specify which one, e.g. \`${item.id}1\`!`, 'bad');
                            else repl(mes.channel, mes.author, `You don't have any ${item.name}!`, 'bad');
                            resolve(false);
                            return;
                        }
                ...
                TudeApi.clubUserByDiscordId(otherPerson.id, otherPerson).then(rec => {
                    TudeApi.performClubUserAction(u, { id: 'transaction', amount: amount, type: type, reciever: rec.id }).then(o => {
                        ...
                        if (o && o.message) repl(mes.channel, mes.author, o.message, 'bad');
                        else repl(mes.channel, mes.author, 'An error occured!', 'bad');
                        resolve(false);
                    });
                }).catch(err => {
