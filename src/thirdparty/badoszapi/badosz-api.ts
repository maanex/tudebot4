...
export default class BadoszAPI {
  ...
  public async toBase64(text: string): Promise<string> {
    const res = await this.makeRequest(BadoszAPI.ENDPOINTS.base64, { text })
    return (await res.json()).formatted
  }

  public async toBinary(text: string): Promise<string> {
    const res = await this.makeRequest(BadoszAPI.ENDPOINTS.binary, { text })
    return (await res.json()).formatted
  }
  ...
  public async getChangeMyMind(text: string): Promise<Buffer> {
    const res = await this.makeRequest(BadoszAPI.ENDPOINTS.changemymind, { text })
    return res.buffer()
  }
  ...
  public async getExcuseme(text: string): Promise<Buffer> {
    const res = await this.makeRequest(BadoszAPI.ENDPOINTS.excuseme, { text })
    return res.buffer()
  }

  public async flipText(text: string): Promise<string> {
    const res = await this.makeRequest(BadoszAPI.ENDPOINTS.flip, { text })
    return (await res.json()).formatted
  }

  public async toHex(text: string): Promise<string> {
    const res = await this.makeRequest(BadoszAPI.ENDPOINTS.hex, { text })
    return (await res.json()).formatted
  }
  ...
  public async toMorseCode(text: string): Promise<string> {
    const res = await this.makeRequest(BadoszAPI.ENDPOINTS.morse, { text })
    return (await res.json()).formatted
  }

  public async reverseText(text: string): Promise<string> {
    const res = await this.makeRequest(BadoszAPI.ENDPOINTS.reverse, { text })
    return (await res.json()).formatted
  }

  public async getSteamUser(id: string): Promise<BadoszAPISteamUser> {
    const res = await this.makeRequest(BadoszAPI.ENDPOINTS.steamUser, { id })
  ...
  public async getTrumpTweet(text: string): Promise<Buffer> {
    const res = await this.makeRequest(BadoszAPI.ENDPOINTS.trump, { text })
    return res.buffer()
  }

  public async toVaporwave(text: string): Promise<Buffer> {
    const res = await this.makeRequest(BadoszAPI.ENDPOINTS.vaporwave, { text })
    return (await res.json()).formatted
  }
